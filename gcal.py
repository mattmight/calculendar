...
# For Google API needs
from apiclient.discovery import build
from httplib2 import Http
from oauth2client import file, client, tools

API_KEYS_DIR = os.getenv("HOME") + '/etc/keys/google-api/'
...
def events_complement(start, events, end):
  if events == []: 
    if start >= end: return []
    else: return [cal_event(start,end)]
...
def events_union(eventsA, eventsB):
  if eventsA == []: return eventsB
  elif eventsB == []: return eventsA
...
class cal_interval:
  ...
  def __invert__(self):
    return cal_interval(self.start, events_complement(self.start, self.events, self.end), self.end)
...
def cal_daily_event(start,end,start_hour,start_min,end_hour,end_min):
  ...
  def events_daily_event(start,end,start_hour,start_min,end_hour,end_min):
    ...
    if end < start:
      ...
      ev = cal_event(start.replace(hour=start_hour,minute=start_min),
                     start.replace(hour=end_hour,  minute=end_min))
      evs = events_daily_event(start.shift(days=+1), end,
                                   start_hour, start_min, 
                                   end_hour, end_min)
      return [ev] + evs
  
  return cal_interval(start,events_daily_event(new_start,new_end,start_hour,start_min,end_hour,end_min),end)  
...
def cal_weekends(start,end):
  ...
  def events_weekends(start,end):
    ...
    if end < start:
      ...
      return [cal_event(start,start.shift(days=+1).floor('day'))] + events_weekends(start.shift(days=+1),end)
    else:
...
parser.add_argument("--start", 
                     default=NOW,
                     help="start of timespan in ISO 8601 format")

parser.add_argument("--end", 
                     default=IN30,
                     help="end of timespan in ISO 8601 format")

parser.add_argument("--output-timezone",
                    default='US/Central',
                    help="timezone to use for output")

parser.add_argument("--query-timezone",
                    default='US/Central',
                    help="timezone to use for output")


parser.add_argument("--busy-calendars",
                    metavar="calendar-id",
                    default=["primary"],
                    nargs="+",
                    help="calendar ids to use for busy information")

parser.add_argument("--free-calendars",
                    metavar="calendar-id",
                    default=[],
                    nargs="+",
...
def get_calendar_events(calendarId='primary', maxResults=10):
  events_result = (
    service.events().list(calendarId=calendarId,
...
def get_freebusy(calendarIds=['primary'],
